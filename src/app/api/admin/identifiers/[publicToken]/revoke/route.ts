...
export async function PATCH(_request: Request, context: RouteContext) {
  try {
    ...
    const identifier = await prisma.childIdentifier.findUnique({
      where: { publicToken },
      select: { id: true, publicToken: true, status: true },
    });
    if (!identifier)
      return NextResponse.json(
        { ok: false, error: "Identificador nao encontrado." },
        { status: 404 },
      );
    if (identifier.status !== IdentifierStatus.ACTIVE) {
      return NextResponse.json(
        { ok: false, error: "Este identificador ja nao esta ativo." },
        { status: 409 },
      );
    }
    ...
    await prisma.$transaction(async (transaction) => {
      ...
      await transaction.auditLog.create({
        data: {
          ...
          description: "Identificador protegido revogado.",
          metadata: { publicToken: identifier.publicToken },
        },
      });
    });
    ...
    return NextResponse.json(
      { ok: false, error: "Nao foi possivel revogar o identificador." },
      { status: 500 },
    );
  }
}
