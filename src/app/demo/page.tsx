    ...
    <main className="app-surface min-h-screen">
      <GsapReveal>
        <section className="border-b border-slate-200 bg-white">
          <div className="mx-auto grid max-w-7xl gap-8 px-4 py-8 sm:px-6 lg:grid-cols-[0.8fr_1.2fr] lg:px-10 lg:py-12">
            <div className="flex flex-col justify-center">
              <div
                data-gsap="hero"
                className="flex w-fit items-center gap-2 rounded-full border border-emerald-200 bg-emerald-50 px-4 py-2 text-sm font-semibold text-emerald-700"
              >
              ...
              <h1
                data-gsap="hero"
                className="mt-5 max-w-2xl text-4xl font-semibold leading-tight tracking-tight text-slate-950 sm:text-5xl"
              >
              ...
              >
                Use os passos abaixo como roteiro: primeiro um evento confiável, depois o
                responsável, em seguida o QR público e por fim o tratamento do alerta.
              </p>
              ...
              {demoSteps.map((step) => {
                ...
                return (
                  <Link
                    ...
                    data-gsap="card"
                    href={step.href}
                    className="dashboard-panel group grid gap-4 rounded-[22px] border border-slate-200 bg-white p-5 shadow-sm transition duration-200 hover:-translate-y-1 hover:border-emerald-200 hover:shadow-xl hover:shadow-slate-900/10 sm:grid-cols-[auto_1fr_auto] sm:items-center"
                  >
                    <div
                      className={`flex h-14 w-14 items-center justify-center rounded-2xl border ${step.accent}`}
                    >
        ...
        <section className="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-10">
          <div
            data-gsap="timeline"
            className="grid gap-5 rounded-[24px] border border-slate-200 bg-slate-950 p-5 text-white sm:grid-cols-[auto_1fr] sm:p-7"
          >
            ...
            <div>
              <h2 className="text-xl font-semibold">
                Frase de sustentação durante a demo
              </h2>
              <p className="mt-2 leading-7 text-slate-300">
                A criança não precisa de celular. O sistema registra eventos de proteção,
                não trajetos. O QR não revela identidade e a localização só vem do
                aparelho de quem decide ajudar.
              </p>
            </div>
          </div>
        </section>
...
function Summary({ value, label }: { value: string; label: string }) {
  return (
    <div className="home-surface rounded-2xl border border-slate-200 bg-slate-50 p-4">
