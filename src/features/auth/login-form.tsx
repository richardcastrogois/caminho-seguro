...
import { ArrowRight, Building2, BusFront, HeartHandshake, ShieldCheck, Users } from "lucide-react";
      ...
      <Card className="rounded-3xl border-sky-100 bg-white/90 shadow-xl shadow-sky-950/5">
        <CardHeader className="gap-3 p-5 sm:p-6">
          ...
          <CardDescription className="text-base leading-7">
            Escolha um papel para mostrar que cada ator ve apenas a parte necessaria do Caminho Seguro. O QR publico continua aberto, sem login e sem dados pessoais.
          </CardDescription>
        </CardHeader>
        <CardContent className="p-5 pt-0 sm:p-6 sm:pt-0">
          <div className="rounded-2xl border border-sky-100 bg-sky-50/80 p-4 text-sm leading-6 text-slate-700">
            Esta tela nao autentica pessoas reais. Ela simula privacidade por perfil para demonstrar o produto no hackathon sem criar atrito durante a avaliacao.
          </div>
        </CardContent>
      </Card>
      ...
      <Card className="overflow-hidden rounded-3xl border-emerald-100 bg-white/90 shadow-xl shadow-emerald-950/5">
        <form action="/api/auth/demo-login" method="post">
          ...
          <CardHeader className="gap-1 p-5 pb-3 sm:p-6 sm:pb-3">
            <CardTitle className="text-2xl text-slate-950">Login de demonstracao</CardTitle>
            <CardDescription>
          ...
          <CardContent className="flex flex-col gap-4 p-5 pt-0 sm:p-6 sm:pt-0">
            {unauthorized && (
              <div className="rounded-2xl border border-amber-200 bg-amber-50 p-3 text-sm font-medium text-amber-900">
                Esse perfil nao acessa a area escolhida. Selecione o perfil correto abaixo.
              </div>
            )}

            <div className="grid gap-3 sm:grid-cols-2">
              <div className="flex min-w-0 flex-col gap-2">
                <Label htmlFor="demo-email">E-mail do perfil</Label>
                <Input id="demo-email" value={selected.email} readOnly className="h-11 min-w-0 rounded-2xl bg-white" />
              </div>

              <div className="flex min-w-0 flex-col gap-2">
                <Label htmlFor="demo-password">Senha simulada</Label>
                <Input id="demo-password" type="password" value="privacidade-simulada" readOnly className="h-11 min-w-0 rounded-2xl bg-white" />
              </div>
            </div>
                  ...
                  >
                    <span className="flex min-w-0 items-center gap-3">
                      ...
                      <span className="min-w-0">
                        <span className="block truncate font-semibold text-slate-950">{profile.label}</span>
                        <span className="block truncate text-xs text-slate-500">{profile.email}</span>
                      </span>
                    </span>
                    <span className="mt-3 block text-sm leading-5 text-slate-600">{profile.description}</span>
                  </button>
                );
              })}
            </div>
          ...
          <CardFooter className="border-t border-slate-100 p-5 sm:p-6">
            <Button type="submit" size="lg" className="h-12 w-full rounded-2xl bg-gradient-to-r from-sky-600 via-cyan-600 to-emerald-600 text-white hover:brightness-105">
